...
namespace eStoreClient.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<Customer> signInManager, ILogger<LoginModel> logger, IConfiguration configuration, UserManager<Customer> userManager)
        {
            ...
            AuthenUrl = "https://localhost:7271/api/Authentication";
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    Customer user = _userManager.FindByEmailAsync(Input.Email).Result;
                    if (user != null)
                    {
                        HttpContext.Session.SetString("USERID", user.Id);
                    }
                    string loginForm = JsonSerializer.Serialize(Input);
                    StringContent content = new(loginForm, Encoding.UTF8, "application/json");
                    HttpResponseMessage response = await _httpClient.PostAsync(AuthenUrl + "/login", content);
                    if (response.IsSuccessStatusCode)
                    {
                        string message = await response.Content.ReadAsStringAsync();
                        string token = JsonSerializer.Deserialize<string>(message, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
